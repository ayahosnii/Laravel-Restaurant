...
class BestSellerComponent extends Component
{
    ...
    public function render()
    {
        ...
        {
            $mealsQuery = $mealsQuery->whereBetween('id',[$this->min_date,$this->max_date])->orderBy('id', 'ASC');
        }
        else if ($this->sorting=="date-desc")
        {
            $mealsQuery = $mealsQuery->whereBetween('id',[$this->min_date,$this->max_date])->orderBy('id', 'DESC');
        }
        else if ($this->sorting=="price")
        {
            $mealsQuery = $mealsQuery->whereBetween('price',[$this->min_price,$this->max_price])->orderBy('price', 'ASC');

        }
        else if ($this->sorting=="price-desc")
        {
            $mealsQuery = $mealsQuery->whereBetween('price',[$this->min_price,$this->max_price])->orderBy('price', 'DESC');
        }
        else if ($this->sorting=="alphabet")
        {
            $mealsQuery = $mealsQuery->whereBetween('name', [$this->min_alphabet,$this->max_alphabet])->orderBy('name', 'ASC');
        }
        else if ($this->sorting=="alphabet-desc")
        {
            $mealsQuery = $mealsQuery->whereBetween('name',[$this->min_alphabet,$this->max_alphabet])->orderBy('name', 'DESC');
        }

        if ($this->categoryInputs) {
            if ($default_lang == 'en') {
                $mealsQuery = $mealsQuery->whereIn('main_cate_id', $this->categoryInputs);
            } else {
                ...
                if ($mealTranslation) {
                    ...
                    if ($meal) {
                        $mealsQuery = $mealsQuery->where('category_id', $meal->category_id);
                    }
                }
            }
        }
        ...
        if ($this->filterProviders) {
            $mealsQuery = $mealsQuery->whereIn('provider_id', $this->filterProviders);
        }
